...
class InstructionFormatter {
  public static format(ix: Instruction, accountMetas: AccountMeta[], idl: Idl): InstructionDisplay | null {
  ...
  private static formatIdlData(idlField: IdlField, data: Object, types?: IdlTypeDef[]): string {
    ...
    if (idlField.type.hasOwnProperty('vec')) {
      return (
        '[' +
        (<Array<IdlField>>data)
          .map((d: IdlField) => this.formatIdlData({ name: '', type: (<IdlTypeVec>idlField.type).vec }, d))
          .join(', ') +
        ']'
      );
    }
    if (idlField.type.hasOwnProperty('option')) {
      return data === null
        ? 'null'
        : this.formatIdlData({ name: '', type: (<IdlTypeOption>idlField.type).option }, data);
    }
  ...
  private static formatIdlDataDefined(typeDef: IdlTypeDef, data: Object, types: IdlTypeDef[]): string {
      ...
      if (typeDef.type.variants[0].name) {
        ...
        const namedFields = Object.keys(enumType)
          .map((f) => {
            ...
            const idlField = variants[variant]?.filter((v: IdlField) => v.name === f)[0];
    ...
    return accounts
      .map((account) => {
        ...
        if (account.hasOwnProperty('accounts')) {
          ...
          return InstructionFormatter.flattenIdlAccounts((<IdlAccounts>account).accounts, newPrefix);
        } else {
          return {
            ...(<IdlAccount>account),
