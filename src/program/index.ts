...
import { utf8 } from '../utils/bytes/index';
...
export class Program<IDL extends Idl = Idl> {
  ...
  public constructor(idl: IDL, programId: Address, provider?: Provider) {
    ...
    // Dynamic namespaces.
    const [account] = NamespaceFactory.build(idl, this._coder, programId, provider);
    //this.rpc = rpc;
   ...
   */
  public static async at<IDL extends Idl = Idl>(address: Address, provider?: Provider): Promise<Program<IDL>> {
   ...
   * @param provider  The network and wallet context.
   */
  public static async fetchIdl<IDL extends Idl = Idl>(address: Address, provider?: Provider): Promise<IDL | null> {
   ...
   *                  program logs.
   */
  public addEventListener(eventName: string, callback: (event: any, slot: number) => void): number {
